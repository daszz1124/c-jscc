...
class ConditionSwinJSCC(nn.Module):
    def __init__(self, args, config):
        super(ConditionSwinJSCC, self).__init__()
        self.config = config
        ...
        if config.logger is not None:
            config.logger.info("modelwork config: ")
    ...
    def forward(self, input_image,embedding_vector, given_SNR=None, given_rate=None):
        ...
        # w/SA 仅适应 SNR/ w/o_SA&RA 无自适应

        if self.model == 'SwinJSCC_w/o_SAandRA' or self.model == 'SwinJSCC_w/_SA':
            feature = self.encoder(
                input_image, chan_param, channel_number, self.model,embedding_vector)
            ...
            if self.pass_channel:
                noisy_feature = self.feature_pass_channel(
                    feature, chan_param, avg_pwr)
            else:
                noisy_feature = feature
