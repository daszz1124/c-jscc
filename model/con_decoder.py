...
import torch
from model.encoder import AdaptiveModulator, SwinTransformerBlock
...
class BasicLayerWithCond(BasicLayer):
    def __init__(self, *args, query_dim=None, **kwargs):
        ...
        self.query_encoder = QueryEncoder(
            query_dim=query_dim, embed_dim=kwargs['dim'])
...
class SwinJSCC_Decoder(nn.Module):
    def __init__(self, img_size, embed_dims, depths, num_heads, C, query_dim,
        ...
        self.patches_resolution = (
            img_size[0] // 2 ** len(depths), img_size[1] // 2 ** len(depths))
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayerWithCond(dim=int(embed_dims[i_layer]),
                                       out_dim=int(
                                           embed_dims[i_layer + 1]) if (i_layer < self.num_layers - 1) else 3,
    ...
    def forward(self, x, snr, model, query):
...
def build_model(config):
    ...
    with torch.no_grad():
        ...
        delta_t = t1 - t0
        print("Decoding Time per img {}s".format(
