...
def parse_args():
    ...
    parser.add_argument('--C', type=str, default='192',
...
def test_epoch(net, test_loader, config, logger, writer, epoch, node_rank, args):
    ...
    epoch_dir = os.path.join(config.samples, f"epoch{epoch}")
                ...
                for batch_idx, batch in enumerate(test_loader):
                    ...
                    if node_rank == 0:
                        test_samples_dir = os.path.join(
                            epoch_dir, f"test_SNR{SNR}_Rate{rate}")
                        ...
                        if args.testset == 'Kodak':
                            ...
                            if psnr > 35:
                                torchvision.utils.save_image(recon_image, os.path.join(
                                    os.path.join(test_samples_dir, dataset_name), image_name))
