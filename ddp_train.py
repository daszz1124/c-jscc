...
def parse_args():
    ...
    parser.add_argument('--training', action='store_true', default=False, help='Training or testing')
    parser.add_argument('--trainset', type=str, default='DIV2K', choices=['CIFAR10', 'DIV2K'], help='Train dataset name')
    parser.add_argument('--testset', type=str, default='kodak', choices=['kodak', 'CLIC21', 'ffhq'], help='Test dataset')
    parser.add_argument('--distortion-metric', type=str, default='MSE', choices=['MSE', 'MS-SSIM'], help='Evaluation metric')
    ...
    parser.add_argument('--C', type=str, default='96', help='Bottleneck dimension (comma-separated for test)')
    parser.add_argument('--multiple-snr', type=str, default='1,4,7,10,14', help='SNR values (comma-separated for test)')
    parser.add_argument('--model_size', type=str, default='base', choices=['small', 'base', 'large'], help='SwinJSCC model size')
    parser.add_argument('--model_path', type=str, default="/home/iisc/zsd/project/VG2SC/SwinJSCC/checkpoint/SwinJSCC_w_SAandRA.model")
    ...
    parser.add_argument('--print_step', type=int, default=100, help='Frequency of logging during training')
...
class Config:
    def __init__(self, args):
        ...
        self.base_path = args.dataset_path if hasattr(args, 'dataset_path') else "/home/iisc/zsd/project/VG2SC/SwinJSCC/datasets/"
        self.test_data_dir = [f"{self.base_path}/kodak/"] if args.testset == 'kodak' else []
        self.train_data_dir = [
            f"{self.base_path}/clic2020/**",
            f"{self.base_path}/clic2021/train",
            f"{self.base_path}/clic2021/valid",
            f"{self.base_path}/clic2022/val",
            f"{self.base_path}/DIV2K_train_HR",
            f"{self.base_path}/DIV2K_valid_HR"
        ]
        self.batch_size = args.batch_size
        self.downsample = 4
        self.isTrain = args.training
        self.channel_number = int(args.C) if args.model in ['SwinJSCC_w/o_SAandRA', 'SwinJSCC_w/_SA'] else None
...
def load_weights(net, model_path, device):
    ...
    pretrained = {k: v for k, v in pretrained.items() if k in model_dict and model_dict[k].size() == v.size()}
...
def train_one_epoch(net, train_loader, optimizer, epoch, config, logger, writer, node_rank, global_step):
    ...
    for batch_idx, input in enumerate(train_loader):
        ...
        global_step[0] += 1
        input = input[0].to(net.device) if isinstance(input, (list, tuple)) else input.to(net.device)
...
def main(opts):
    ...
    train_dataset, test_dataset = select_dataset(opts, config)
    ...
    net = DDP(net, device_ids=[device_id], output_device=device_id, find_unused_parameters=True)

    # Optimizer
    model_params = [{'params': net.parameters(), 'lr': config.learning_rate}]
    optimizer = optim.Adam(model_params, lr=config.learning_rate)
    lr_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[1000, 2000, 3000], gamma=0.5)
...
def distributed_init(opts):
    ddp_url = getattr(opts, "ddp.dist_url", f"tcp://localhost:{opts.dist_port}")
    node_rank = getattr(opts, "ddp.rank", 0)
    world_size = torch.cuda.device_count()
    
    dist.init_process_group(backend="nccl", init_method=ddp_url, world_size=world_size, rank=node_rank)
...
if __name__ == "__main__":
    ...
    torch.multiprocessing.spawn(fn=distributed_worker, args=(main, opts), nprocs=num_gpus)